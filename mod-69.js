...
function makePossiblePairs(arr, cond) {
  ...
  res.forEach(el => {
    const digit = table_of_weights.get(el.toString())
      ? table_of_weights.get(el.toString())
      : table_of_weights.get(el.reverse().toString());
    result.push(digit);
  });
...
function rFourFunc(m) {
  ...
  mCopy = [...m[0].map((el, index) => [mCopy[0][index], mCopy[1][index]])].flat();

  for (let i = 0; i < mCopy.length; i++) {
    for (let j = 0 + i; j < mCopy.length; j++) {
      const numOfGroup = group1.includes(mCopy[i])
        ? 1
        : group2.includes(mCopy[i])
        ? 2
        : group3.includes(mCopy[i])
        ? 3
        : null;
...
function rSevenFunc(m) {
  ...
  for (let key in prom_obj) {
    const res = [[], []];
    let shouldDelete = key
      .split(',')
      .map(el => +el)
      .every(el => prom_obj[key].includes(el));
    if (!shouldDelete) {
      prom_obj[key] = [];
    } else {
